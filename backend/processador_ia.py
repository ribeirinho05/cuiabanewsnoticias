"""
CUIABÁ NEWS — Processador IA (Google Gemini 2.5 Flash via HTTP direto)
...
logger = logging.getLogger('cuiabanews.processador_ia')
...
def processar_lote(noticias, model=None):
    ...
    for i, noticia in enumerate(noticias):
        ...
        try:
            ...
            quota_esgotada = True
            logger.warning(f"  ⚠️ Quota diária atingida na notícia {i+1}/{total}.")
            noticia['resumo'] = noticia.get('texto', noticia.get('titulo', ''))[:300]
            noticia['ia_processada'] = False
            continue
        time.sleep(1)
...
def gerar_trending_topics(noticias, model=None):
    ...
    return ['#CuiabáNews', '#MatoGrosso', '#Brasil', '#Economia', '#Política', '#Esportes']
